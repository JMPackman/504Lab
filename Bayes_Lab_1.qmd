...
We've gone from a full data set with $N = 344$ rows, to a subset with $n = 68$ rows. ("\$" signs hold LaTex snippets)
...
The histogram of body_mass_g, which is centered around approximately 3750g, would be normally distributed if not for a gap between 3750-4000g. The distribution of bill_length_mm is bimodal, with peaks around 46-47mm and 52mm.
...
### Question 1.2: Can you predict what the mean value, and standard deviations will be? Why? Calculate it. Compare this against outputs in summary(fit1.ols) and explain. Map the values you find to the latex equations before. –Come back to this!!!!!!
...
It's going through the process of 4 (I expect Markov?) chains, each with 2000 iterations.
...
The intercepts are roughly the same, and brm() has estimated the predictor body_mass_g as 0, which is close to the estimate provided by lm(). Instead of p-values, brm() provides 95% CI. Also, brm() does not provide residual se, multiple or adjusted R2, but does provide estimate for the distributional parameter sigma.
...
```{r, warning = F}
bind_rows(
  tidy(fit1.ols, conf.int = TRUE) %>% dplyr::select(term, estimate, contains("conf")),
...
min(gentoo$body_mass_g)
max(gentoo$body_mass_g)

#plottingwith predict 
nd2 <- tibble(body_mass_g = seq(from = 3950,
                               to = 6300,
                               length.out = 50))
...
```{r}
cbind(coef(fit2.ols),              # point estimates
      sqrt(diag(vcov(fit2.ols))),  # standard errors
      confint(fit2.ols))           # 95% CIs
...
Diagonal: Density estimates for each parameter.

Off-Diagonal: Are these the correlations between parameters?
...
The posterior gives us a credible (high density) intervals for representing uncertainty. Posterior distributions are analagous to OLS' 95% CI around point estimates.
...
The skew in sigma (i.e., the posterior distribution for sigma is skewed relative to the prior) might arise because the likelihood for a given value of sigma was also skewed. IS this what's referred to as the likelihood "overcoming" the prior?
...
Given a skewed parameter, predicting based on the mode might be wise, as the median or mean might be biased by a few unusually high or low draws.
...
I think it's unlikely we have been doing with without any sort of prior. Rather than a flat or uniform prior, we have been using a weakly informative prior, which allows us to realistically constrain the space of all possible parameter values without needing to know too much about the prior; rather, we can specify the distribution and use some starting values that will not bias our estimates too much.
...
glimpse(draws2)
draws2 <- draws2 %>% 
  mutate(gentoobeta0 = b_Intercept,
         gentoobeta1 = b_body_mass_g)

draws2 %>% 
  select(.draw, gentoobeta0, gentoobeta1) %>% 
  mutate(gentoobody_mass_g = mean(gentoo$body_mass_g)) %>% 
  mutate(gentooy_hat = gentoobeta0 + gentoobeta1 * gentoobody_mass_g) %>% 
  ggplot(aes(x = gentooy_hat)) +
  geom_histogram(bins = 40) +
  labs(title = "Gentoo posterior distributions",
       x = expression(hat(italic(y))*'|'*italic(x)==3733.1)) +
  coord_cartesian(xlim = c(47, 51))
#Not sure why this plot isn't coming out right. 

```

```{r, warning=FALSE}
draws2 %>% 
  rename(`beta[0]` = gentoobeta0,
         `beta[1]` = gentoobeta1) %>% 
  pivot_longer(cols = c(`beta[0]`, `beta[1]`, sigma), 
               names_to = "parameter") %>% 
...
draws2 %>% 
  ...
  mutate(gentooy_hat = gentoobeta0 + gentoobeta1 * body_mass_g) %>% 
  
  ggplot(aes(x = body_mass_g, y = gentooy_hat, group = .draw, color = gentoobeta0)) +
  geom_line() +
  scale_color_viridis_c(expression(beta[0]~(the~intercept)), end = .9)
```

I'm not sure why two of the plots aren't appearing. Perhaps it's a rendering issue.
