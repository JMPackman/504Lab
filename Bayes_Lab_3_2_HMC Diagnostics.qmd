...
The `pairs()` plot is a wrapper around the `mcmc_pairs()` function from **bayesplot**. By default, half of the chains are depicted in the scatter plots below the diagonal, and the other half are displayed above the diagonal. The basic idea is you want the results from different chains to mirror one another. You can control this behavior with the `condition` argument.
...
```{r}
as_draws_df(fit13.b) %>% 
  # notice the 3 meta-data columns at the end: Chain, iteration, and draw
  glimpse()
```
...
Including the warmup draws results in weird, jagged trace plots at the first 10 or so iterations, I assume because the model has not yet converged at that point?
...
Lag 0 autocorrelation has to be 1 because it's measuring the correlation between a Markov chain value (the first one?) and itself. But, autocorrelation quickly drops off, meaning chain values that are farther apart (in terms of steps or lags) are not strongly correlated, which means the MCMC is mixing well. An undesirable autocorrelation plot would have increases or would have high values for autocorrelation across lags, indicating poor mixing.
