...
-   the likelihood distributions: P(D\|θ)
-   the prior parameter distribution (aka priors): $P_0$(θ)
-   the prior predictive distributions: $P_0$(D)?
-   the posterior parameter distributions (aka posteriors): P(θ\|D)
-   the posterior-predictive distribution: P(D)?
...
Discuss how the jagged lines have to do with the uncertainty in $\sigma$. Is this a note to you or a question to us? I would imagine the more jagged the lines, more uncertainty in sigma?
...
```{r fit3.b, message = F, warning = F, results = "hide"}
#renaming to fit3.b to avoid confusion with my gentoo model
fit3.b <- brm(
  data = chinstrap,
  bill_length_mm ~ 1 + body_mass_g,
...
### Answer: It looks like the priors are pretty much the same. What is the purpose of the lb = 0 argument? 
...
```{r}
posterior_summary(fit2.b)

b0    <- posterior_summary(fit2.b)[1, 1]
b1    <- posterior_summary(fit2.b)[2, 1]
sigma <- posterior_summary(fit2.b)[3, 1]
```

```{r}
crossing(body_mass_g    = seq(from = 3500, to = 7000, length.out = 200),
         bill_length_mm = seq(from = 35, to = 60, length.out = 200))  %>% 
  mutate(density = dnorm(x = bill_length_mm, 
                         mean = b0 + b1 * body_mass_g,
                         sd = sigma)) %>% 
...
as_draws_df(fit2.b) %>% 
  ...
  scale_y_continuous(NULL, breaks = NULL) +
  labs(title = "fit2.b",
       subtitle = "95% intervals in histograms",
       x = "parameter space") +
  facet_wrap(~ parameter, scales = "free", labeller = label_parsed)
...
fitted(fit2.b, 
       ...
       summary = F) %>% 
  data.frame() %>% 
  set_names(pull(nd2, body_mass_g)) %>% 
  mutate(draw = 1:n()) %>% 
  pivot_longer(-draw) %>% 
  mutate(body_mass_g = as.double(name)) %>%
...
```{r}
# save the fitted() results
f2 <- fitted(fit2.b, newdata = nd2) %>% 
  data.frame() %>% 
  bind_cols(nd2) 

predict(fit2.b, newdata = nd2) %>% 
  data.frame() %>% 
  bind_cols(nd2) %>% 
