---
title: "Packman_Lab06_Intro to MLM Exercise/Walkthrough"
params: 
    SHOW_SOLS: TRUE
    TOGGLE: TRUE
editor_options: 
...
Analysis question: How does practice influences the reading age of toy characters
...
We get plots for each type of toy, which have the same regression line for each toy, which fits some toys better than others. This is probably because the model doesn't include toy type. 
...
###All three lines at once, because why not
ggplot(toy2, aes(x = hrs_week)) + 
  geom_point(aes(y = R_AGE), size=1, alpha=.3) +
  facet_wrap(~toy_type) +
  geom_line(aes(y=pred_fe), col = "blue") + geom_line(aes(y=pred_ri), col = "green") +
  geom_line(aes(y=pred_rs), col = "orange")  

```

The first model adjusts for the toy-type. which doesn't change the slope of the line, but it shifts it vertically. The second model looks the same, but it's because it adds a random intercept for toy type, which is like a different starting point when x, or hrs/week, is 0.Essentially, this allows the average reading age to vary by toy. Beyond that, I'm not sure how it differs from the first model. 
The third model looks like a random intercept and a random slope for hrs/week, which allows the relationship between reading age and hrs/week to vary by group (toy-type). 
...
#I did not appreciate how the second (green model) differed from the first (blue model) in that it modeled a distribution of deviations from an average, Is that the overall sample average? 
...
#A1 represents the standard deviation of the distances between the toy type lines (or their intercepts?) to the black line (representing the overall FE of hrs/week).
#A2 represents sd of the distances between each data point of each toy type to the lines for each toy type. 
#A3 is the intercept for the black line
#A4 the slope of the black line (FE for hrs/week for the all toys combined?). 
...
A1 is $\sigma_{\varepsilon}$  
A2 is $\sigma_{\varepsilon}$
A3 is $\gamma_{00}$  
A4 is $b_{1}$
...
`r qbegin(qcounter())`
How many participants are there in the data?  30 participants.
How many have complete data (15 trials)?  No participant has complete data.
What is the average number of trials that participants completed? What is the minimum? Avg = 12, Min = 10. 
Does every participant have _some_ data for each type of audio? Yes, each participant has data from at least two trials for each audio type. 
...
head(efdat)
n_distinct(efdat$PID) #30 participants 

efdat %>% 
  count(PID) %>%
  summary() #On average, P's completed 12 trials (min = 10; max = 14).

table(efdat$PID, efdat$audio) #P's have data for ≥2 trials for each audio type.
```
`r solend()`
...
- What variables are we seeking to investigate in terms of their impact on the outcome? Audio. type, headphones, and maybe an audio X headphones interaction.
...
Audio type. Each participant hears audio of all types.
- What varies *between* these clusters?  
Headphone use. Each participant either used speakers or noise-cancelling headphones. 
...
We want to test the effect of audio on SDMT, and whether that effect depends on heaphone usage, which implies an interaction.
...
There are three audio types, two media (headphones or speakers), and 30 participants. Audio and heaphones are fixed effects. SO that indicates we should include by-participant random intercepts, at the very least. 
...
There might be a random by-participant effect of audio, i.e., some could find it more disturbing than others. It would be interesting to test random by-participant effects of headphones, but we don't have that in this data because participants either used headphones or speakers, so there's no effect "by-participant."

```{r}
#Model specification

efmlm1 <- lmer(SDMT ~ audio*headphones + (1+audio | PID), data = efdat)
summary(efmlm1)
...
#Or, equivalently:
efmlm1 <- lmerTest:: lmer(SDMT ~ audio*headphones + (1+audio | PID), data = efdat)
summary(efmlm1)
#Note: Your "efmlm1" is the same as "sdmt_mod" in the solutions. 
```
...
anova(sdmt_res, sdmt_mod)

#How does SATmodcomp differ from ANOVA? Does a significant result in SATmodcomp indicate that the larger model is better? 
#Note to self: Remember this package (and the citation) to use with your economic games models! 
```
...
#Solution attempt
library(effects)
effect(term = "audio*headphones", mod = efmlm1) %>%
  as.data.frame() %>%
  ggplot(aes(x=audio,y=fit,
             ymin=lower,ymax=upper,
             col=headphones))+
  geom_pointrange(size=1,lwd=1)
#Note: Use the solution for help on aesthetics (like size). 
```
...
Linear mixed-effects regression modeled SDMT scores. Fixed effects included (1) type of audio (no audio, white noise, or music), with no audio as the reference category; (2) audio delivery medium (speakers, which was the reference level, vs noise-canceling headphones, henceforth abbreviated “headphones”), and (3) the interaction of audio type and audio delivery medium. This interaction term addresses whether the effect of audio type on SDMT scores (i.e., measure of executive function) differed based on audio delivery method (i.e., whether or not the audio is heard via noise-canceling headphones vs speakers). Model results appear in Table 1 below. 

Random effects included participant-level random intercepts as well as random slopes for type of audio (each participant had at least two trials with each type of audio). 

Fitting models with lme4 and using REML estimations, model comparison between the full model with the audio type X audio delivery method interaction and a restricted model without the interaction term (but otherwise identical) indicated that including the interaction term improved goodness of fit (F(2,26.9 = 11.05, p < 0.001). This result indicates that the impact of audio type on SDMT depends on the method of audio delivery (headphones vs speakers). 

Main effects:
Participants who did not wear headphones and encountered no audio stimuli (i.e., reference levels for both audio type and delivery method, henceforth referred to as “the reference group”) had an average SDMT score of approximately 33.26 (i.e., the model intercept). 

Participants who did not wear headphones, but heard music (via speakers) had significantly lower SDMT scores than those in the reference group (b = -8.02; t(27.59) = -5.68; p < 0.001). Listening to white noise did not demonstrate an effect: there was no significant difference in SDMT scores between listening to white noise and no audio.

Without audio playing, wearing headphones predicted significantly higher SDMT scores, relative to those without headphones (b = 6.85; t(28.19) = 2.44; p = 0.021). 

Interaction: 
The effect of hearing music on SDMT scores did not significantly depending on headphone wearing (b = -3.59; t(27.94) = -1.79; p = 0.084). 

Relative to the reference group (speakers, not headphones, and no audio) listening to white noise through headphones predicted further increase in SDMT scores (b = 8.02; t(26.46) = 3.92; p < 0.001). Figure 1 displays these findings. 
...
#Figure 1
#| label: fig-efplot
#| fig-cap: "Interaction between audio type (no audio/white noise/music) and delivery method (speakers/ANC headphones) on SDMT scores"
#| echo: false
plotfit <- effect(term = "audio*headphones", mod = sdmt_mod) %>%
  as.data.frame()

ggplot(efdat, aes(x=audio,y=SDMT,col=headphones))+
  geom_jitter(height=0,width=.2,alpha=.25) +
