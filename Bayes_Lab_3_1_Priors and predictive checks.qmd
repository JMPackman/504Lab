...
Is this because we have not yet chosen our priors?
...
The second prior is a generic, weakly informative prior. The first is a more specific informative prior. My understanding of exponential priors is that they're best-suited to rates. But I don't think that means any of these priors is theoretically impossible.

Since the scale values are positive, a theoretically impossible prior would be one where sigma could be negative.

Also, I was reading about Popoviciu's inequality, though I'm not sure that applies here. If so, because the data are bounded \[1, 10\], then sigma cannot be greater than ((10-1)^2^ / 4) ^1/2^ = 4.5.
...
The code specifies candidate priors for beta0 and sigma, then iterating those density distributions across different values of bty_avg.
...
The first prior predictive, where beta0 is centered at 5.5 and sigma is an exponential prior with a rate of 1/1, is preferable because it respects the upper an lower bounds more (not perfectly, but better than the second prior). Also, it seems implausible that beta0 would be centered at 8 on the bty_avg scale, as that's one of the highest values possible (and the highest value actually represented in the dataset), so it seems unlikely (unless the data is from Los Angeles or television).
...
The prior predictive check makes sure that the data generated by the priors is appropriate/makes sense, so we can make sure we have reasonably informative priors. The posterior predictive check makes sure that the replicated data, generated according to posterior predictive distribution, fits the data fairly well.
...
Now we update our by-hand prior predictive simulation to accommodate $\beta_0$ and $\beta_1$.
...
The posterior predictive check looks sort of alright. The distributions for the most part respect the upper and lower boundaries, and roughly match the shape of the observed data. It doesn't appear that gender has a significant relationship with bty_avg.
